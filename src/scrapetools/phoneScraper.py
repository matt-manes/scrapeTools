...
def getNumConsecutiveNumbers(text:str, reverse:bool=False)->int:
    ...
    maxToCheck = 10
    while i < len(text):
        checkdex = -i-1 if reverse else i
        if not text[checkdex].isnumeric():
            return i
        else:
            i += 1
        if i > maxToCheck:
    ...
    if count > 0:
        ...
        for _ in range(count):
            ...
            if sepdex != -1:
                ...
                startOffset = getNumConsecutiveNumbers(text[lastStopdex:sepdex], reverse=True)
                #consecutive numbers between sepdex and nextSepdex
                firstStopOffset = getNumConsecutiveNumbers(text[sepdex+1:nextSepdex+1])
                ...
                if startOffset == 3 and firstStopOffset == 3 and secondStopOffset == 4:
                    #xxx{separator}xxx{separator}xxxx
                    number = text[sepdex-startOffset:nextSepdex+secondStopOffset+1]
                elif startOffset == 0 and firstStopOffset == 3 and secondStopOffset == 4\
                and text[sepdex-1] == ')' and text[sepdex-5] == '(':
                    #(xxx){separator}xxx{separator}xxxx
                    number = text[sepdex-5:sepdex+firstStopOffset+secondStopOffset+2]
...
def findByHref(text:str)->list[str]:
    ...
    for _ in range(count):
        ...
        if any(prefix in number for prefix in prefixes):
            number = ''.join([num for num in number[number.find(':')+1:] if num.isnumeric()])
            if len(number) == 10:
                numbers.append(number)
    return numbers
...
def scrapePhoneNumbers(text:str)->list[str]:
    ...
    numbers = [number for number in numbers if phonenumbers.is_valid_number(phonenumbers.parse('+1'+number))]
