...
def validate(email:str)->bool:
    ...
    if all([atdex < lastDot,
            ...
            not all(ch.isnumeric() 
                    for ch in domain.replace('.','')),
            not all(ch.isnumeric() 
                    for ch in local.replace('.','')),
            all(email[i-1] != '.' 
                for i,ch in enumerate(email) if ch == '.'),
            all(ext not in domain 
                for ext in ['.png', '.jpg', '.js',
                            '.html', '.svg', '.jpeg', 
                            '.mp4', '.mpeg', '.css', 
                            '.pdf', '.wav', '.docx', 
                            '.txt', '.rtf', '.gif',
...
def findLastValidCharacterOffset(text:str)->int:
    ...
    if text[-1] == '@' and text[0] != '@':
        ...
        raise ValueError('First or last character of text arg needs to be "@"\n',
